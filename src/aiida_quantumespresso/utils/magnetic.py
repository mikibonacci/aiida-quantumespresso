    ...
    def generate_magnetic_namelist(self, parameters):
        ...
        if parameters["SYSTEM"].get("nspin",None) == 2:
            ...
            if self.structure.is_collinear:
                for kind,magmom in zip(self.structure.properties.kinds, self.structure.properties.magmoms):
                    # this should be fixed, now only magmom_z is considered...
                    if magmom[2] != 0:
                        namelist["starting_magnetization"][kind] = magmom[2]
            else:
                raise NotImplementedError("The input structure is not collinear, but you choose collinear calculations.")
        elif parameters["SYSTEM"]["noncolin"]:
            for site in self.structure.properties.sites:
